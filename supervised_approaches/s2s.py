...
class Seq2Seq:
    def __init__(self, batch_size, epochs, latent_dim, num_samples, data_path, model_file=None):
    ...
    def prepareData(self):
        ...
        self.encoder_input_data = np.zeros(
            (len(self.input_texts), max_encoder_seq_length, self.num_encoder_tokens), dtype="float32"
        )
        self.decoder_input_data = np.zeros(
            (len(self.input_texts), self.max_decoder_seq_length, self.num_decoder_tokens), dtype="float32"
        )
        self.decoder_target_data = np.zeros(
            (len(self.input_texts), self.max_decoder_seq_length, self.num_decoder_tokens), dtype="float32"
        )

        for i, (input_text, target_text) in enumerate(zip(self.input_texts, target_texts)):

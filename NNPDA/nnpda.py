...
variables_dict = {"Ws": tf.Variable(tf.random.normal([Ns, Ns, Nr, Ni]),
                                    dtype=tf.dtypes.float32,
                                    name="state_weights"),
                  # Weight matrix for computing internal state.
                  "bs": tf.Variable(tf.ones([Ns, 1]), dtype=tf.dtypes.float32,
                                    name="state_bias"),
                  # Bias vector for computing internal state.
                  "Wa": tf.Variable(tf.random.normal([2 ** Ns, Nr, Ni]),
                                    dtype=tf.dtypes.float32,
                                    name="action_weights"),
                  # Weight matrix for computing stack action.
                  "ba": tf.Variable(tf.ones([1, 1]), dtype=tf.dtypes.float32,
                                    name="action_bias")}  # Scalar bias for computing stack action.


class NnpdaCell:
    def __init__(self, state_weights, state_bias, action_weights, action_bias,
                 delta, delta_, input_symbol=None,
                 current_state=None, current_stack=None,
    ...
    def __call__(self, **kwargs):
        ...
        WI_s = tf.tensordot(self.state_weights, self.input_symbol,
                            axes=1)  # The product Ws*I     shape [Ns x Ns x Nr]
        ...
        WIR_s = tf.reduce_sum(
            input_tensor=tf.tensordot(WI_s, self.current_stack, axes=1),
            axis=-1)  # The product Ws*I*R   shape [Ns x Ns]
        # WIR_s = tf.tensordot(WI_s, self.current_stack, axes=1)
        # print("WIR_s shape", tf.shape(WIR_s), "should be Ns x Ns",end="\n\n")

        WIRS = tf.tensordot(WIR_s, self.current_state,
                            axes=1)  # The product Ws*I*R*S shape [Ns x 1]
        ...
        WIRS_bias = tf.transpose(tf.nn.bias_add(tf.transpose(WIRS),
                                                tf.reshape(self.state_bias, [
                                                    -1])))  # Adding the state bias shape [Ns x 1]
        # print("WIRS_bias shape", tf.shape(WIRS_bias), "should be Ns x 1",end="\n\n")

        next_state = self.state_activation(
            WIRS_bias)  # Applying the activation function shape [Ns x 1]
        ...
        WI_a = tf.tensordot(self.action_weights, self.input_symbol,
                            axes=1)  # The product Wa*I    shape [2^Ns x Nr]
        # print("WI_a shape", tf.shape(WI_a), "should be 2^Ns x Nr",end="\n\n")

        WIR_a = tf.reduce_sum(
            input_tensor=tf.tensordot(WI_a, self.current_stack, axes=1),
            axis=-1)  # The product Wa*I*R  shape [2^Ns]
        ...
        Sdelta = tf.multiply(self.delta, tf.transpose(
            a=tf.reverse(self.current_state, axis=[
                1])))  # The product delta*S          shape [2^Ns x 1]
        # print("Sdelta shape", tf.shape(Sdelta), "should be 2^Ns x 1 (?)",end="\n\n")

        Sdelta_ = tf.multiply(self.delta_, tf.transpose(
            a=tf.reverse(1 - self.current_state, axis=[
                1])))  # The product (1-delta)*(1-S)  shape [2^Ns x 1]
        # print("Sdelta_ shape", tf.shape(Sdelta_), "should be 2^Ns x 1 (?)",end="\n\n")

        P = tf.reduce_prod(input_tensor=Sdelta + Sdelta_,
                           axis=1)  # P matrix  shape [2^Ns x 1]
        # print("P shape", tf.shape(P), "should be 2^Ns x 1",end="\n\n")

        # Equation 23 and Equation 5b continued

        WIRP = tf.tensordot(WIR_a, P,
                            axes=1)  # Scalar stack action value (NOTE: WRIP is already a scalar, no reduction needed)
        ...
        stack_axn = self.action_activation(
            WIRP)  # Applying the activation function
...
nnpda_cycle(Ns, Ni, Nr, Na, batch_size, num_steps, str_len, optimizer=RMSprop,
            activation=sigmoid)

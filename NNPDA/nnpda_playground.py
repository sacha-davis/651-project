...
class NnpdaCell:
    ...
    def nnpda_cycle(self,Ns, Ni, Nr, Na, batch_size, num_steps, str_len, optimizer=RMSprop, activation=sigmoid):
        # cell = NnpdaCell
        ...
        self.current_state = tf.ones([Ns, 1])

        self.delta, self.delta_ = get_delta(Ns)
        ...
        self.state_weights=variables_dict["Ws"]
        self.state_bias=variables_dict["bs"]
        self.action_weights=variables_dict["Wa"]
        self.action_bias=variables_dict["ba"]
        # self.delta=delta
        # self.delta_=delta

...
class nnpda_cell(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, Ws, st_bias,
                 Wa, axn_bias, delta, delta_, input_sym=None, curr_state=None,
    ...
    def __call__(self):
        with vs.variable_scope(scope or type(self).__name__):
            WI_s = tf.reduce_sum(tf.tensordot(self.Ws, input_sym, axes=1),
                                 axis=-1)  # The product Ws*I     shape [Ns x Ns x Nr]
            WIR_s = tf.reduce_sum(tf.tensordot(WI_s, self.curr_read, axes=1),
                                  axis=-1)  # The product Ws*I*R   shape [Ns x Ns]
            WIRS = tf.tensordot(WIR_s, self.curr_state,
                                axes=1)  # The product Ws*I*R*S shape [Ns x 1]
            WIRS_bias = tf.nn.bias_add(WIRS,
                                       self.st_bias)  # Adding the state bias            shape [Ns x 1]
            next_state = self.st_activation(
                WIRS_bias)  # Applying the activation function shape [Ns x 1]

            WI_a = tf.reduce_sum(tf.tensordot(Wa, self.input_sym, axes=1),
                                 axis=-1)  # The product Wa*I    shape [2^Ns x Nr]
            WIR_a = tf.reduce_sum(tf.tensordot(WI_a, self.curr_read, axes=1),
                                  axis=-1)  # The product Wa*I*R  shape [2^Ns]

            Sdelta = tf.multiply(self.delta, tf.transpose(
                tf.reverse(self.curr_state,
                           dims=1)))  # The product delta*S          shape [2^Ns x 1]
            Sdelta_ = tf.multiply(self.delta_, tf.transpose(
                tf.reverse(1 - self.curr_state,
                           dims=1)))  # The product (1-delta)*(1-S)  shape [2^Ns x 1]
            P = tf.reduce_prod(Sdelta + Sdelta_,
                               axis=1)  # P matrix                     shape [2^Ns x 1]

            WIRP = tf.reduce_sum(tf.tensordot(WIR_a, P),
                                 axis=-1)  # Scalar stack action value
            WIRP_bias = tf.nn.bias_add(WIRP,
                                       self.axn_bias)  # Adding the scalar action bias
            stack_axn = self.axn_activation(
                WIRP_bias)  # Applying the activation function
...
def nnpda_full_3rd_order(
    ...
    for i in range(num_steps):
        ...
        next_state, stack_axn = cell(words[:, i],
                                     ...
                                     axn_bias=axn_bias,
                                     delta=delta,
                                     delta_=one_minus_delta)
        curr_state = next_state

    # Computing the Loss E = (Sf - S(t=T))^2 + (L(t=T))^2
    loss_per_example = tf.square(tf.norm(st_desired - curr_state)) + tf.square(
        len_stack.peek())
